...
// ── 🔴 LFS guard ───────────────────────────────────────────────────────────
// raw.githubusercontent.com serves LFS POINTER files (~130 bytes), not the shard
// bytes, so a single .gitattributes routing *.bin through LFS would make every
// fetch silently return garbage — the failure looks like a corrupt index, not a
// hosting mistake. Hard-fail if the destination repo carries ANY .gitattributes
// (working tree at any depth, or .git/info/attributes). See the NEVER-LFS banner
// above; RUN-20260717-1507 risk #4.
function findGitattributes(root) {
    const hits = [];
    if (!existsSync(root)) return hits;
    const infoAttr = join(root, '.git', 'info', 'attributes');
    if (existsSync(infoAttr)) hits.push(infoAttr);
    const stack = [root];
    while (stack.length) {
        const d = stack.pop();
        let entries;
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) {
                if (e.name === '.git') continue; // .git internals handled via info/attributes
                stack.push(p);
            } else if (e.name === '.gitattributes') {
                hits.push(p);
            }
        }
    }
...
function main() {
    ...
    // 🔴 LFS guard — fail BEFORE touching the destination if any .gitattributes
    // is present (it would silently route shard fetches through LFS pointers).
    const attrHits = findGitattributes(DEST);
    if (attrHits.length) {
        console.error('🔴 LFS GUARD: .gitattributes present in the publish destination:');
        for (const a of attrHits) console.error(`   ${a}`);
        console.error('raw.githubusercontent.com does not resolve LFS pointers — it would serve');
        console.error('~130-byte pointer files instead of shard bytes, and every fetch would');
        console.error('silently return garbage. Remove all .gitattributes (and any LFS filter');
        console.error('config) from the destination repo before publishing. NEVER enable Git LFS here.');
        process.exit(1);
    }
...
- \`bigram/shards/XX/YY-<hash6>.bin\` — bigram + English word-term postings
  (\`manifest.shardCount\` buckets, FNV-1a32 mod shardCount — read the manifest, do not assume 4096).
- \`bigram/unigram/XX/YY-<hash6>.bin\` — unigram postings (same shardCount).
- \`english.jsonl\` — English/Latin query corpus (transitional; retired once the
  bigram index carries English word terms, manifest \`wordTerms: true\`).
- \`INDEX_VERSION.json\` — version stamp. **Lock to \`commit_sha\`**: fetch this file
  from \`main\`, read \`commit_sha\`, then fetch \`manifest.json\` / \`docs.txt\` / every
  shard pinned to THAT commit. \`index_id\` is observability only — it is blind to
  \`docs.txt\`, the file that maps docIds to documents, so it cannot guard the pairing.
...
`);
    ...
    // Pick one real shard path for the post-publish magic-byte check below.
    let sampleShardRel = null;
    if (manifest.shards && typeof manifest.shards === 'object' && !Array.isArray(manifest.shards)) {
        for (const [bucketKey, hash6] of Object.entries(manifest.shards)) {
            if (hash6 && hash6 !== '0' && /^[0-9a-f]{4}$/.test(bucketKey)) {
                sampleShardRel = `bigram/shards/${bucketKey.slice(0, 2)}/${bucketKey.slice(2)}-${hash6}.bin`;
                break;
            }
        }
    }

    const textNote = manifest.textShards == null
        ? 'text/ not built (SKIP_TEXT_SHARDS) — verified via TEI at the consumer'
        : 'text/ excluded from the publish — verified via TEI at the consumer';

    console.log(`Published to: ${DEST}`);
    console.log(`  files : ${files}`);
    console.log(`  size  : ${(bytes / 1048576).toFixed(1)} MB   (${textNote})`);
    console.log(`  index_id: ${stamp.index_id}   docs: ${stamp.docs}   ` +
        `shardCount: ${manifest.shardCount}   wordTerms: ${manifest.wordTerms === true}`);
    console.log('');
    ...
    console.log('');
    console.log('── Post-publish check (LFS/IIDX): a shard must serve real bytes, not a pointer.');
    console.log('   Fetch one shard raw and confirm the first 4 bytes are the magic "IIDX":');
    if (sampleShardRel) {
        console.log(`     curl -s "https://raw.githubusercontent.com/<OWNER>/<REPO>/$SHA/${sampleShardRel}" | head -c 4`);
    } else {
        console.log('     curl -s "https://raw.githubusercontent.com/<OWNER>/<REPO>/$SHA/bigram/shards/XX/YY-<hash6>.bin" | head -c 4');
    }
    console.log('   Expected output: IIDX   (anything else — e.g. "vers" from an LFS pointer — means the fetch is broken).');
}
