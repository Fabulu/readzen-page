...
/**
 * Best-effort workId/fileId extraction from a title record. Mirrors the
 * helper that lived in views/search.js, lifted here so search-side callers
 * can reuse it without depending on the view layer.
 */
export function getWorkId(t) {
    if (!t) return '';
    const path = (t.path || t.Path || '').toString();
    return (t.fileId || t.fileID || t.workId || t.WorkId || fileIdFromPath(path) || '');
}
