 ...
 */
export function renderLinesHtml(lines, segmentMap) {
    return lines
        .filter((line) => line)
        .map((line) => {
            ...
            return `<div class="line-row${segClass}" data-line-id="${id}"${segAttr}${speakerAttr}><span class="line-id">${id}</span><span class="line-text">${content}</span></div>`;
        }).join('');
}
