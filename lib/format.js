...
export function renderLinesHtml(lines) {
    return lines.map((line) => {
        const content = line && line.text
            ? escapeHtml(line.text)
            : '<span class="line-empty">[empty line]</span>';
        const id = line ? escapeHtml(line.id) : '';
        return `<div class="line-row"><span class="line-id">${id}</span><span class="line-text">${content}</span></div>`;
    }).join('');
}
