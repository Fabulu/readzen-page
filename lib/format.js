...
export function renderLinesHtml(lines, segmentMap, opts) {
    ...
    return lines
        ...
        .map((line) => {
            ...
            return `<div class="line-row${segClass}" data-line-id="${id}"${segAttr}${speakerAttr}><span class="line-id" title="${id}">${id}</span><span class="line-text">${content}</span>${linkBtn}</div>`;
        }).join('');
}
