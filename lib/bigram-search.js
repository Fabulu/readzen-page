...
/**
 * Text-shard bucket count from the manifest (CONTRACT §1: clients read the
 * shard count from the manifest, NEVER hardcode 4096). Returns:
 *   - a positive integer  → that many text shards were published; bucket a
 *                           docId with `docId % count`.
 *   - null                → the publish ships NO text shards
 *                           (`textShards: null` — the scoped Devvit build).
 *                           Callers MUST skip text-shard verification instead
 *                           of issuing one doomed 404 fetch per bucket.
 * A manifest that OMITS the field entirely (legacy v3 manifests, and the test
 * fixtures) is treated as the historical 4096-shard layout, so SPA production
 * (shards present) and the existing suite stay byte-identical.
 * @param {object|null} manifest
 * @returns {number|null}
 */
function textShardCountFromManifest(manifest) {
    const ts = manifest ? manifest.textShards : undefined;
    if (ts === null) return null; // explicit: no text shards published → skip
    if (ts && typeof ts.count === 'number' && ts.count > 0) return ts.count;
    return TEXT_SHARD_COUNT; // absent/malformed → legacy 4096-shard layout
}
...
export async function verifyDocPhrase(docId, query, opts = {}) {
    ...
    const cjkNeedles = extractCjkRuns(normalizeString(query == null ? '' : String(query))).runs;
    // Finding #2: an en/community doc stores englishNormalize'd text, which
    // PRESERVES inline CJK verbatim — so it can match the CJK leg of a mixed
    // query via inline Chinese, with no English word overlap at all. Verify it
    // against the UNION of both needle classes (word ∪ CJK), not word-only:
    // otherwise a doc that genuinely matched on Chinese is scored 0 on the
    // absent word and wrongly dropped (and its docId marked dead for the
    // session). Source docs store CJK-normalized text with no English, so they
    // stay CJK-needles-only (adding word needles would zero every source row).
    const wordNeedles = isEnglishSide ? extractWordRuns(query) : [];
    const needles = isEnglishSide ? wordNeedles.concat(cjkNeedles) : cjkNeedles;
    if (needles.length === 0) return null;

    // Finding #3: derive the text-shard bucket count from the manifest, never a
    // hardcoded 4096 (CONTRACT §1). On a `textShards: null` publish (the scoped
    // Devvit build ships no text/) skip text-shard verification — return null
    // (could-not-verify) so the caller keeps its index estimate — instead of
    // issuing a doomed 404 per bucket. A manifest-load failure falls back to
    // the legacy 4096 layout, byte-identical to the historical no-manifest path
    // so the existing verifyDocPhrase suite (which mocks no manifest) is
    // unchanged.
    let shardCount = TEXT_SHARD_COUNT;
    try {
        const manifest = await loadManifest(signal);
        const c = textShardCountFromManifest(manifest);
        if (c === null) return null; // no text shards published → cannot verify
        shardCount = c;
    } catch (err) {
        if (isAbortError(err)) return null;
        // Manifest unavailable → legacy default (byte-identical fallback).
    }

    let docMap;
    try {
        const bucketStr = (docId % shardCount).toString(16).padStart(3, '0');
    ...
    if (isEnglishSide) {
        // OR-presence over the union: the doc reached display via WORK-level AND
        // (a sibling doc carries the missing needle class), so it need NOT
        // contain every needle. Count every real hit across both scripts and
        // drop only when the doc contributes NOTHING (finding #2). This keeps
        // an inline-CJK-only match alive with an honest count, and — when the
        // word IS present — no longer discards the doc's CJK hits.
        let total = 0;
        for (const needle of needles) total += countSubstringHits(text, needle);
        return total; // 0 only when neither script appears → genuine non-match
    }
    // Source (CJK) side: historical phrase-ish AND — every CJK run must appear
    // or the doc is a verified 0. Byte-identical to the prior behavior.
    let total = 0;
...
export async function searchFulltext(query, opts = {}) {
    ...
    // Finding #3: read the text-shard bucket count from the manifest, not a
    // hardcoded 4096 (CONTRACT §1). `textShards: null` means the publish ships
    // no text/ — verification is impossible, so surface the index candidates
    // unverified (hitCount 1) rather than fire a doomed 404 per bucket. In
    // practice this branch is unreachable: a no-text-shard publish is the
    // word-capable (v4) Devvit build, which is served entirely by
    // searchWordCapable above and never reaches this v2 fallback. SPA/v2
    // indexes always ship text shards, so the count resolves to 4096 (absent →
    // legacy default) or the manifest's explicit count — byte-identical.
    const textShardCount = textShardCountFromManifest(manifest);
    if (textShardCount === null) {
        const unverified = [];
        for (let i = 0; i < verifyCount; i++) unverified.push({ docId: candidates[i], hitCount: 1 });
        if (onProgress && unverified.length) {
            try { onProgress(unverified.slice()); } catch (_) { /* ignore consumer errors */ }
        }
        emitStats(2, candidates.length, unverified.length, candidates.length > _verificationCap);
        return unverified;
    }
    ...
    for (let i = 0; i < verifyCount; i++) {
        ...
        const bucketStr = (docId % textShardCount).toString(16).padStart(3, '0');
