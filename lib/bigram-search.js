...
export async function fileIdForDocId(docId) {
    ...
    if (!Number.isInteger(docId) || docId < 0 || docId >= list.length) return '';
    const url = list[docId];
    if (!url) return '';
    // Last non-empty path segment is the fileId.
...
export async function searchFulltext(query, opts = {}) {
    ...
    // Verification step: cap to top 200 by intersect-order, fetch each
    // candidate's text shard, and count true substring hits.
    ...
    for (let i = 0; i < verifyCount; i++) {
        ...
        const bucketStr = (docId % 256).toString().padStart(2, '0');
        const list = candidatesByBucket.get(bucketStr);
        if (list) list.push(docId);
        else candidatesByBucket.set(bucketStr, [docId]);
    }

    // Fetch text shards in parallel (they're 2.5 MB each; AbortSignal cancels
    // mid-flight). On error per shard, skip those candidates with a warning.
    const bucketEntries = Array.from(candidatesByBucket.entries());
    let textShardMaps;
    try {
        textShardMaps = await Promise.all(
            bucketEntries.map(([bucketStr]) => loadTextShard(bucketStr, signal))
        );
    } catch (err) {
        if (isAbortError(err)) return [];
        throw err;
    ...
    for (let i = 0; i < bucketEntries.length; i++) {
        const [, docIds] = bucketEntries[i];
        const docMap = textShardMaps[i];
        if (!docMap) continue; // 404'd shard
        for (const docId of docIds) {
            const text = docMap.get(docId);
            if (!text) continue;
            const hits = countSubstringHits(text, normalizedQuery);
            if (hits > 0) verified.push({ docId, hitCount: hits });
        }
    }
...
async function loadBigramShard(bucketId, manifest, signal) {
    ...
    // Single-flight: dedupe concurrent fetches for the same bucket.
    if (_inflightBigramShards.has(bucketId)) {
        return _inflightBigramShards.get(bucketId);
    }
    ...
    const promise = (async () => {
        ...
        if (!hash || hash === '0') {
            // Empty shard: no postings for any term in this bucket.
    ...
    _inflightBigramShards.set(bucketId, promise);
    return promise;
}
...
async function loadTextShard(bucketStr, signal) {
    ...
    if (_inflightTextShards.has(bucketStr)) {
        return _inflightTextShards.get(bucketStr);
    }
    ...
    _inflightTextShards.set(bucketStr, promise);
    return promise;
}
