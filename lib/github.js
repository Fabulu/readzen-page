...
/**
 * Raw-GitHub URL for a data-repo file (e.g. `zen_texts.json`) as of a specific
 * git ref. `DATA_REPO_BASE` points at the `main` branch; this swaps `main` for
 * the given commit SHA (or any ref) so a caller can fetch a file exactly as it
 * existed at that commit — the mechanism behind the canon view's time-travel.
 * With an empty/falsey ref it returns the current (`main`) URL unchanged.
 *
 * @param {string} ref   Commit SHA, tag, or branch. Empty → current `main`.
 * @param {string} name  Data-repo-relative file name, e.g. `zen_texts.json`.
 * @returns {string}     Absolute raw.githubusercontent.com URL.
 */
export function dataFileUrlAtRef(ref, name) {
    const file = String(name || '').replace(/^\/+/, '');
    if (!ref) return DATA_REPO_BASE + file;
    const base = DATA_REPO_BASE.replace(/\/main\/$/, '/' + encodeURIComponent(ref) + '/');
    return base + file;
}
...
/**
 * Fetch aggregated star counts from star-counts.json (one file, one fetch).
 * The desktop app writes this file during sync. Returns a Map keyed by
 * `fileId:translator` with integer counts. Cached per session.
 */
...
export async function fetchStarCounts(corpus) {
    ...
    _starCountsCache[key] = counts;

    try {
        const base = key === 'open' ? OPEN_DATA_REPO_BASE : DATA_REPO_BASE;
        const text = await fetchText(base + 'star-counts.json');
        const data = JSON.parse(text);
        for (const [mapKey, count] of Object.entries(data)) {
            if (typeof count === 'number' && count > 0) {
                counts.set(mapKey, count);
            }
        }
    } catch { /* file doesn't exist yet or parse error — no stars to show */ }
...
export function loadTranslatedFileIds() {
    ...
    _translatedPromise = (async () => {
        const ids = new Set();
        const repos = [
            { url: 'https://api.github.com/repos/Fabulu/CbetaZenTranslations/git/trees/main?recursive=1', prefix: 'xml-p5t/', corpus: 'cbeta' },
            { url: 'https://api.github.com/repos/Fabulu/OpenZenTranslations/git/trees/main?recursive=1', prefix: 'xml-open-t/', corpus: 'openzen' },
        ];
            ...
            try {
                ...
                for (const entry of data.tree) {
                    if (entry.path.startsWith(repo.prefix) && entry.path.endsWith('.xml') && entry.type === 'blob') {
                        const relPath = entry.path.substring(repo.prefix.length);
                        const parts = relPath.split('/').filter(Boolean);
                        if (parts.length === 0) continue;
                        const fname = parts[parts.length - 1];
                        const baseId = fname.replace(/\.xml$/i, '');
                        if (!baseId) continue;
                        if (repo.corpus === 'openzen') {
                            // OpenZen: canonical fileId is `<topDir>.<basename>`
                            // (e.g. `ws.gateless-barrier`). Without the publisher
                            // prefix the EN badge in browse mode mis-matches.
                            if (parts.length < 2) continue;
                            ids.add(parts[0] + '.' + baseId);
                        } else {
                            // CBETA: fileId IS the basename (e.g. `T48n2005`).
                            ids.add(baseId);
                        }
                    }
                }
