...
export function highlightTextInHtml(html, term) {
    ...
    return html.replace(
        /(<span class="line-text"[^>]*>)([\s\S]*?)(<\/span>)/g, // attr-tolerant: merged-mode spans carry data-line-id
 ...
 */
export function scrollToLineId(container, lineId, opts) {
    if (!container || !lineId) return;
    const quiet = !!(opts && opts.quiet); // resume: scroll without the highlight pulse
    setTimeout(() => {
        const el = container.querySelector(`[data-line-id="${CSS.escape(lineId)}"]`);
        if (el) {
            if (!quiet) el.classList.add('scroll-target');
            el.scrollIntoView({ behavior: quiet ? 'auto' : 'smooth', block: quiet ? 'start' : 'center' });
        }
    }, 500);
}
