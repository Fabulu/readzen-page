...
export function attachInlineDict(container) {
    ...
    if (supportsHover) {
        container.addEventListener('pointermove', onContainerHover);
...
function onContainerHover(evt) {
    ...
    hoverTimer = setTimeout(() => doLookup(evt.clientX, evt.clientY), 300);
}
...
async function onContainerClick(evt) {
    ...
    clearTimeout(hoverTimer);
    doLookup(evt.clientX, evt.clientY);
}
...
async function doLookup(x, y) {
    ...
    showPopup(matchedTerm, matchedEntries, x, y);
}

/**
 * Create and position the popup element showing the lookup card.
 */
async function showPopup(term, entries, clickX, clickY) {
    ...
    if (supportsHover) {
        ...
        popup.addEventListener('pointerleave', () => {
            leaveTimer = setTimeout(dismiss, 400);
        });
    }
