...
function onContainerPointerDown(evt) {
    ...
    longPress.timer = setTimeout(() => {
        ...
        armClickSuppression();
        doLookup(x, y, false).catch(() => { /* swallow async errors */ });
    }, LONG_PRESS_MS);
}
...
async function doLookup(x, y, fromHover = false) {
    // Respect user preference (hover only; clicks always work)
    if (fromHover && localStorage.getItem('readzen-hover-dict') === 'off') return;

    const info = caretInfoFromPoint(x, y);
    if (!info || info.node.nodeType !== Node.TEXT_NODE) {
        if (!fromHover || popupFromHover) dismiss();
        return;
    }
...
async function showPopup(term, entries, clickX, clickY, isHover = false) {
    ...
    const asDrawer = !isHover && detectCoarsePointer();
