...
function showZenPanel(entry, term) {
    ...
    if (entry) {
        ...
        empty.className = 'zen-panel__empty';
        empty.style.cssText = 'padding:1rem 1.1rem;color:var(--muted);font-size:0.9rem;line-height:1.5';
        empty.textContent = 'No published dictionary entry for ' + (term || 'this term') + '.';
 ...
 */
export async function showZenTerm(term, x, y, shouldShow) {
    ...
    if (thisGeneration !== clickGeneration) return;
    // Caller-supplied guard (e.g. the graph card was dismissed mid-resolve):
    // bail without opening the panel for a context that no longer exists.
    if (typeof shouldShow === 'function' && !shouldShow()) return;
    if (!entry) { showZenPanel(null, term); return; }
