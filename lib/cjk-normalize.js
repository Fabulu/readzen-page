...
const STRIP_SET = (() => {
    ...
    // Stripped: any UTF-16 surrogate code unit in U+DB00..U+DFFF (covers high
    // surrogates AND every Ext-B/C/D low surrogate). Mirrors C# bug for parity.
...
/**
 * BMP CJK ideographs: Unified (U+4E00..U+9FFF), Extension A (U+3400..U+4DBF),
 * and Compatibility (U+F900..U+FAFF). Mirrors desktop's
 * Services/InvertedSearchIndex.IsIndexable.
 */
export function isCjk(cp) {
    return (cp >= 0x4E00 && cp <= 0x9FFF) ||  // CJK Unified Ideographs (BMP)
           (cp >= 0x3400 && cp <= 0x4DBF) ||  // CJK Unified Ideographs Extension A
           (cp >= 0xF900 && cp <= 0xFAFF);    // CJK Compatibility Ideographs
}

/** True if string contains at least one BMP CJK ideograph (per `isCjk`). */
