 ...
 * Uses `location.origin + location.pathname` as the base so it works
 * on any deployment (readzen.pages.dev, localhost, custom domain).
...
export function buildShareableUrl(route) {
    ...
    return base + '#/' + route.rawRoute.replace(/^\/+/, '');
}
