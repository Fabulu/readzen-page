...
//   - As a belt-and-suspenders backup, every active mirror is also tracked in
//     a module-level registry so `detachAllMirrors()` can tear them down on
//     route changes — `selectionchange` may never fire on a navigation, so
//     the self-detach path alone leaks until the next selection.
//   - Spacer rows (`__lg_break_*`, `__pb_break_*`) have no `data-line-id`, so
//     they're naturally excluded from the row walk.

// Active disposers across all attachSelectionMirror calls. Used by
// detachAllMirrors() so app.js can flush every mirror on hashchange.
const _activeDisposers = new Set();
...
function clearAll(panes) {
    for (const pane of panes) {
        ...
        const active = pane.root.querySelectorAll('.is-active'); // rows OR merged inline spans
...
export function attachSelectionMirror(panes) {
    ...
    const handler = () => {
        ...
        for (const pane of panes) {
            ...
            const stale = pane.root.querySelectorAll('.is-active'); // rows OR merged inline spans
            for (const el of stale) el.classList.remove('is-active');
            for (const id of ids) {
                // CSS.escape is unavailable in some test envs; an unescaped id
                // containing a special char would throw SyntaxError out of
                // querySelector. Wrap so a single bad id can't kill the loop.
                let el = null;
                try {
                    const safe = (typeof CSS !== 'undefined' && CSS.escape) ? CSS.escape(id) : id;
                    el = pane.root.querySelector(`[data-line-id="${safe}"]`);
                } catch { el = null; }
        ...
        const sourceStale = sourcePane.root.querySelectorAll('.is-active');
    ...
    const detach = () => {
        ...
        detached = true;
        document.removeEventListener('selectionchange', handler);
        _activeDisposers.delete(detach);
        clearAll(panes);
    };
    _activeDisposers.add(detach);
    return detach;
}

/**
 * Tear down every mirror attached so far. Safe to call from a hashchange
 * handler regardless of how many views registered mirrors.
 */
export function detachAllMirrors() {
    for (const d of Array.from(_activeDisposers)) {
        try { d(); } catch {}
    }
