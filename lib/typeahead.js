...
export function initTypeahead(input, { titles, masters, onSelect }) {
    ...
    function render(q) {
        ...
        // Titles — top 5 substring + master-associated texts
        let tHits = tBlobs.filter(b => b.blob.includes(lq)).slice(0, 5);
        // If masters matched, also find texts by master's Chinese names
        if (mHits.length > 0 && tHits.length < 5) {
            const tPaths = new Set(tHits.map(h => h.fileId));
            for (const mh of mHits) {
                for (const name of (mh.m.names || [])) {
                    if (!name || name.length < 2) continue;
                    const ln = name.toLowerCase();
                    for (const tb of tBlobs) {
                        if (tPaths.has(tb.fileId)) continue;
                        if (tb.blob.includes(ln)) {
                            tHits.push(tb);
                            tPaths.add(tb.fileId);
                            if (tHits.length >= 5) break;
                        }
                    }
                    if (tHits.length >= 5) break;
                }
