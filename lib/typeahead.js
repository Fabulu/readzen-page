// lib/typeahead.js — Standalone typeahead/autocomplete module (zero deps except escapeHtml).
...
export function initTypeahead(input, { titles, masters, onSelect }) {
    ...
    function select(el) {
        ...
        close();
        input.value = '';
        onSelect({ kind, href, query: kind === 'fulltext' ? el.dataset.query : undefined });
    }
    ...
    function render(q) {
        ...
        if (tHits.length) {
            ...
            for (const h of tHits) {
                ...
                const html = `<div class="ta-item ta-item--text" role="option" aria-selected="false" data-kind="text" data-href="${href}"><span class="ta-item-zh">${escapeHtml(h.t.zh || '')}</span><span class="ta-item-en">${escapeHtml(h.t.en || '')}</span></div>`;
                dropdown.insertAdjacentHTML('beforeend', html);
            }
        }
