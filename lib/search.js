...
import { searchFulltext, fileIdForDocId, metaForDocId, getManifestInfo } from './bigram-search.js';
...
/**
 * Full-text search router. Never throws.
 *
 * When the deployed index is word-capable (manifest v4 `wordTerms: true`) the
 * XOR router is gone: ONE engine (lib/bigram-search.js) answers BOTH scripts —
 * CJK bigrams/unigrams AND English word terms, with work-level AND for mixed
 * queries and density ranking. On an older index (no `wordTerms`) we fall back
 * to the historical split: CJK → bigram index, everything else → english.jsonl.
 *
 * The `opts.onProgress` callback (if provided) fires once per result batch with
 * the partial result rows so the UI can render incrementally.
 */
async function searchFullText(q, filters, allowedFileIds, titleData, translatedIds, zenIds, opts = {}) {
    if (!q) return [];
    try {
        // Capability probe (cached manifest — free after the first query).
        let wordCapable = false;
        try {
            const info = await getManifestInfo();
            wordCapable = !!(info && info.wordTerms);
        } catch (_) {
            wordCapable = false; // no index / manifest unavailable → fallback
        }
        if (wordCapable) {
            // One engine, both scripts.
            return await searchFullTextIndexed(q, filters, allowedFileIds, titleData, translatedIds, zenIds, opts);
        }
        // No-capability fallback: the historical XOR router.
        if (containsCjk(q)) {
            return await searchFullTextIndexed(q, filters, allowedFileIds, titleData, translatedIds, zenIds, opts);
        }
...
/** Full-text via the bigram inverted index (lib/bigram-search.js). Handles
 *  CJK on any index and, on a word-capable index, English word terms + mixed
 *  queries too — the backend returns work-level-AND, density-ranked rows. */
async function searchFullTextIndexed(q, filters, allowedFileIds, titleData, translatedIds, zenIds, opts = {}) {
    const titleIndex = buildTitleIndex(titleData);
    const filtersActive = hasActiveFilter(filters);
    const onProgress = typeof opts.onProgress === 'function' ? opts.onProgress : null;

    // Convert one bigram-backend batch ({docId, hitCount, density?}[]) into
    // result-row shape, applying filters + B6 fail-closed + URL synthesis.
    const rowsForBatch = async (batch) => {
        const rows = [];
        for (const { docId, hitCount, density } of batch) {
            ...
            // Density ranking key from the word-capable backend (§4.3); the
            // engine already returns rows in this order. Passed through for any
            // consumer that wants to preserve/replay it. Absent on the fallback.
            if (typeof density === 'number') row.density = density;
            rows.push(row);
        }
        return rows;
    };
