...
import { DATA_REPO_BASE, sourceXmlUrl, authoritativeTranslationUrl, fetchText } from './github.js';
...
async function searchFullTextCjk(q, filters, allowedFileIds, titleData, translatedIds, zenIds, opts = {}) {
    ...
    const hits = await searchFulltext(q, { signal: opts.signal, onProgress: innerOnProgress });
...
export async function loadAndSearchXml(fileId, term, opts = {}) {
    ...
    if (opts && opts.includeTranslation && passages.length > 0) {
        result.translatedPassages = await buildAlignedTranslation(fileId, passages);
    }
 ...
 */
async function buildAlignedTranslation(fileId, passages) {
    const out = new Map();
    ...
    const { linesById: tLinesById, lineOrder: tLineOrder } = tParsed;
    // Build lb → index map once for fast range lookups.
    const tIdxOf = new Map();
    for (let i = 0; i < tLineOrder.length; i++) {
        tIdxOf.set(tLineOrder[i], i);
    }

    for (const p of passages) {
        const startIdx = tIdxOf.get(p.startLb);
        const endIdx = tIdxOf.get(p.endLb);
        if (startIdx == null || endIdx == null) continue;
        const lo = Math.min(startIdx, endIdx);
        const hi = Math.max(startIdx, endIdx);
        const parts = [];
        for (let i = lo; i <= hi; i++) {
            const id = tLineOrder[i];
