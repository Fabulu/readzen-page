...
async function searchFullText(q, filters, allowedFileIds) {
    ...
    try {
        ...
        // Mirror the index-time CJK tokenization (build-pagefind-index.js):
        // each CJK ideograph becomes its own token. Otherwise Jieba would
        // segment 無門 as one token but the index has it as 無 + 門, so 0 hits.
        const tokenizedQuery = q.replace(/([㐀-䶿一-鿿豈-﫿])/g, '$1 ').trim();
        const search = await pf.search(tokenizedQuery, { filters: pfFilters });
        const top = search.results.slice(0, 300);
        const loaded = await Promise.all(top.map(r => r.data()));

        // Phrase filter for CJK 2+ char queries: index-time content has spaces
        // injected after every CJK ideograph, so the tokenized query is itself
        // a phrase pattern when checked against r.content. (r.content from
        // Pagefind contains our injected spaces — checking for the tokenized
        // query string IS an adjacency check.)
        const isCjkPhrase = q.length >= 2 && /[㐀-䶿一-鿿豈-﫿]/.test(q);
        const filtered = isCjkPhrase
            ? loaded.filter(r => (r.content || '').includes(tokenizedQuery))
            : loaded;
        ...
        console.log(`[search] q=${q} tok="${tokenizedQuery}" unfiltered=${search.unfilteredResultCount} raw=${search.results.length} loaded=${loaded.length} phrase=${results.length}`);
