...
async function searchFullText(q, filters, allowedFileIds) {
    ...
    try {
        ...
        // For CJK queries, Pagefind tokenizes per-character and may return
        // documents containing all characters separately rather than as a phrase.
        // Post-filter to require the exact phrase in excerpt or content.
        const isCjk = /[\u3000-\u9fff\uf900-\ufaff]/.test(q);
        const results = [];
        for (const r of loaded) {
            ...
            if (isCjk && q.length >= 2) {
                const plain = (r.excerpt || '').replace(/<[^>]+>/g, '');
                const content = r.content || '';
                if (!plain.includes(q) && !content.includes(q)) continue;
            }
            results.push(entry);
        }
        console.log(`[search] Pagefind returned ${search.results.length} raw, loaded ${loaded.length}, after CJK filter: ${results.length}`);
