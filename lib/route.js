...
//
// Route kinds (history-mode URLs, hash fallback for legacy links):
 ...
 * Prefers pathname (history-mode), falls back to hash for backward compat
 * with old `#/...` links that may still be bookmarked or shared.
...
export function getRawRoute() {
    ...
    // Backward compat: if we're at "/" but the hash has a route, use it
...
export function parseRoute(rawRoute) {
    ...
    switch (first.toLowerCase()) {
        ...
        case 'scholar': {
            ...
            const isGraph = segs[2] && segs[2].toLowerCase() === 'graph';
            return {
                kind: isGraph ? 'scholar-graph' : 'scholar',
                collectionId: segs[1] ? safeDecode(segs[1]) : '',
                passageId: isGraph ? '' : (segs[2] ? safeDecode(segs[2]) : ''),
