...
//     (read top-to-bottom, like a stele's disciple roster) instead of one wide
//     horizontal row — breadth becomes depth;
...
const STACK_MIN = 2;               // >= this many sibling leaves fold vertical
const STACK_COL_ROWS = 6;          // target rows per stack column
...
export function computeLayout(nodes, edges) {
    ...
    for (const p of nodes) {
        ...
        const leaves = kids.filter(k =>
            childrenOf.get(k).length === 0 && !k.isSource && !k.capsule &&
            !sourcesOf.has(k));   // a master with a book shelf never stacks
        if (leaves.length < STACK_MIN) continue;
        const cols = Math.max(1, Math.min(STACK_COLS_MAX,
            Math.ceil(leaves.length / STACK_COL_ROWS)));
        const rows = Math.ceil(leaves.length / cols);
        leaves.sort((a, b) => (a.year || 9999) - (b.year || 9999) ||
            (a.id || '').localeCompare(b.id || ''));
        leaves.forEach((k, i) => {
            // column-major: each column reads top -> bottom
            const col = Math.floor(i / rows), row = i % rows;
            k.stack = { parent: p, col, row, cols, rows };
    ...
    function layoutStack(leaves) {
        ...
        for (const k of leaves) {
            k.x = k.stack.col * colPitch;
            contourAdd(contour, k.layer,
                k.x - NODE_W / 2 - STACK_GUT, k.x + NODE_W / 2);
        }
        let mn = Infinity, mx = -Infinity;
        for (const [, e] of contour) { mn = Math.min(mn, e.min); mx = Math.max(mx, e.max); }
        return { list: leaves.slice(), contour, anchor: (mn + mx) / 2 };
    }
    ...
    function layoutSubtree(n) {
        ...
        if (stacked.length) place(layoutStack(stacked), Math.sqrt(stacked.length));
