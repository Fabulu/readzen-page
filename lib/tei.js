...
export function parseTei(xmlText) {
    ...
    function walk(node) {
        ...
        if (localName === 'head' || localName === 'jhead') {
            // <cb:jhead> = juan/fascicle title. Without this branch it fell
            // through to the default recurse: the title text still rendered,
            // but merged layouts (which break out headings by lineId)
            // swallowed fascicle boundaries into body paragraphs.
            const parts = [];
            ...
            // Also let the contents flow into the normal line buckets
        ...
        if (localName === 'byline') {
            // Editorial attributions (e.g. 參學比丘彌衍宗紹編). Collected so
            // merged layouts can break them out instead of gluing them into
            // the neighboring paragraph; text still flows into line buckets.
            const parts = [];
            collectHeadingText(node, parts);
            const text = normalizeText(parts.join(''));
            if (text) bylines.push({ lineId: current.id || '', text });
            for (let i = 0; i < node.childNodes.length; i += 1) {
                walk(node.childNodes[i]);
            }
            return;
        ...
        // <anchor> — record position for back-matter apparatus linkage.
        if (localName === 'anchor') {
            const xmlId = node.getAttributeNS('http://www.w3.org/XML/1998/namespace', 'id') || node.getAttribute('xml:id');
            if (xmlId && xmlId.startsWith('beg')) {
                if (current.id) ensureLine(current.id);
                const bucket = current.id ? linesById.get(current.id) : null;
                anchorPositions.set(xmlId, { lineId: current.id || '', pos: bucket ? bucket.text.length : 0 });
            }
            return;
        }
    ...
    // Extract witness labels from header <listWit>
    const witnessMap = new Map();
    const listWits = doc.getElementsByTagName('listWit');
    if (listWits.length > 0) {
        const witnesses = listWits[0].getElementsByTagName('witness');
        for (let i = 0; i < witnesses.length; i++) {
            const w = witnesses[i];
            const wId = w.getAttributeNS('http://www.w3.org/XML/1998/namespace', 'id') || w.getAttribute('xml:id');
            if (wId) witnessMap.set(wId, w.textContent.trim());
        }
    }

    walk(body);

    // Process stand-off apparatus from <back>
    const textEl = body.parentNode;
    const back = textEl ? (Array.from(textEl.children || textEl.childNodes).find(n => n.localName === 'back' || n.nodeName === 'back')) : null;
    if (back) {
        const apps = back.getElementsByTagName('app');
        for (let i = 0; i < apps.length; i++) {
            const appNode = apps[i];
            const from = (appNode.getAttribute('from') || '').replace(/^#/, '');
            const anchor = anchorPositions.get(from);
            if (!anchor) continue;
            let lem = '';
            const rdgs = [];
            for (let j = 0; j < appNode.childNodes.length; j++) {
                const child = appNode.childNodes[j];
                if (child.nodeType !== 1) continue; // ELEMENT_NODE only
                const parts = [];
                collectPlainText(child, parts);
                const txt = parts.join('');
                if (child.localName === 'lem') lem = txt;
                else if (child.localName === 'rdg') {
                    rdgs.push({ wit: child.getAttribute('wit') || '', text: txt });
                }
            }
            if (lem || rdgs.length) {
                apparatus.push({ lineId: anchor.lineId, pos: anchor.pos, lem, readings: rdgs });
            }
        }
    }
    ...
    const { license, licenseUrl } = extractLicense(doc);
    ...
    return {
        ...
        linesById,
        lineOrder,
        headings: dedupeAdjacentHeadings(headings),
        bylines,
        apparatus,
        witnessMap,
        license,
...
/**
 * Render-layer dedupe for the heading stream (shared by the reader body's
 * heading styling via `headingIds`, and any TOC built from `extractHeadings`).
 *
 * CBETA TEI legitimately repeats a section heading — e.g. T48n2005 (無門關) has
 * two consecutive <cb:div type="xu"> preface divisions BOTH headed 禪宗無門關 — so
 * a heading list built straight from the <head> stream shows the same heading
 * twice in a row. The corpus XML is canonical and never edited, so we collapse
 * the redundant REPEAT here.
 *
 * Rule (conservative, mirrors the desktop HeadingDeduplicator): collapse a run
 * of CONSECUTIVE heading entries that share the same `level` AND
 * whitespace-normalized, exactly-equal `text`. The FIRST entry survives (keeping
 * its `lineId` navigation target); the immediately-following identical repeats
 * are dropped. Same text at a different level, or separated by any different
 * heading, stays distinct — we never merge two genuinely separate sections that
 * merely share a title far apart.
 */
export function dedupeAdjacentHeadings(headings) {
    if (!Array.isArray(headings) || headings.length === 0) return [];
    const out = [];
    let prevKey = null;
    let prevLevel = null;
    for (const h of headings) {
        const key = String(h.text || '').replace(/[\s　]+/g, ' ').trim();
        if (out.length > 0 && h.level === prevLevel && key === prevKey) {
...
/**
 * Extract license metadata from the TEI header. Looks at the standard places:
 *   <availability>/<licence target="...">label</licence>
 *   <availability status="...">free text</availability>
 * Returns { license: '', licenseUrl: '' } when nothing usable is found —
 * callers treat empty values as "license unspecified, render nothing" so
 * this stays a fail-open lookup.
 */
function extractLicense(doc) {
    const fileDesc = doc.getElementsByTagNameNS(TEI_NS, 'fileDesc')[0];
    if (!fileDesc) return { license: '', licenseUrl: '' };
    const availabilities = fileDesc.getElementsByTagNameNS(TEI_NS, 'availability');
    if (!availabilities || !availabilities.length) return { license: '', licenseUrl: '' };
    const avail = availabilities[0];

    // Prefer an explicit <licence> child (TEI P5 element name).
    const licences = avail.getElementsByTagNameNS(TEI_NS, 'licence');
    if (licences && licences.length) {
        const lic = licences[0];
        const label = normalizeText(lic.textContent || '');
        const url = lic.getAttribute('target') || lic.getAttribute('href') || '';
        if (label || url) return { license: label, licenseUrl: url };
    }

    // Fallback: any text content directly in <availability>. Strip nested
    // <p> noise by joining text nodes we can see.
    const text = normalizeText(avail.textContent || '');
    if (text) {
        // Try to find an obvious URL inside the text.
        const m = text.match(/https?:\/\/\S+/);
        const url = m ? m[0] : '';
        return { license: text, licenseUrl: url };
    }
