...
export function parseTei(xmlText) {
    ...
    function walk(node) {
        ...
        // suppress alternate readings from the display text.
        if (localName === 'rdg') {
            if (current.inApp) return;
        ...
        // <lg> — verse group. No special content handling needed; just
        // recurse into children. The <lb> elements inside provide line
        // separation.
        if (localName === 'lg') {
            for (let i = 0; i < node.childNodes.length; i += 1) {
                walk(node.childNodes[i]);
