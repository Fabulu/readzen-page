...
function senseHtml(sense, entryId, senseIndex) {
    ...
    // Related term links: #/term/{term}. The href keys on the CHINESE head term
    // (the lookup key); the visible label defaults to English once the headword
    // index arrives — see applyRelatedTermGlosses, which rewrites the label to
    // "english gloss · 術語" via the data-zen-related-term attribute.
    const termLinks = (sense.relatedTerms || []).map((t) =>
        `<a class="zen-link" data-zen-related-term="${escapeHtml(t)}" href="#/term/${encodeURIComponent(t)}">${escapeHtml(t)}</a>`);
    if (termLinks.length) {
