...
/** Add a text to a named list. */
export function addToList(listName, fileId, title) {
    const lists = _load();
    if (!lists[listName]) lists[listName] = [];
    if (lists[listName].some((i) => i.fileId === fileId)) return;
    lists[listName].push({ fileId, title, addedAt: Date.now() });
    _save(lists);
}
...
/** Record the user's last-read position. */
export function setLastRead(fileId, title, scrollPercent) {
    try {
        localStorage.setItem(LAST_READ_KEY, JSON.stringify({
            fileId, title, scrollPercent, timestamp: Date.now()
        }));
    } catch {}
}
