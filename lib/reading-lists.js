...
const SUPPRESS_KEY = 'readzen-no-continue';

/** Record the user's last-read position with the full route. */
export function setLastRead(fileId, title, scrollPercent, route, lineId) {
    try {
        if (localStorage.getItem(SUPPRESS_KEY)) return;
        localStorage.setItem(LAST_READ_KEY, JSON.stringify({
            fileId, title, scrollPercent, route: route || fileId,
            lineId: lineId || '', timestamp: Date.now()
        }));
    } catch {}
}

/** Clear last-read position permanently until user reads something new via a direct action. */
...
/** Re-enable tracking (called when user explicitly opens a text via bookmark or Start Here). */
export function resumeLastReadTracking() {
