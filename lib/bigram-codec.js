 ...
 *          `length` is byte length of the varint run; `count` is recovered
 *          by counting varint terminator bytes (high-bit-clear) in the
 *          posting byte run via _countVarints.
 * @throws {Error} when buffer < 16 bytes, magic ≠ "IIDX", or version ≠ 2.
 */
export function readShardHeader(bytes) {
    ...
    for (let i = 0; i < termCount; i++) {
        ...
        terms.set(e.term, {
            ...
            count: _countVarints(bytes, absOffset, e.postingByteLen),
        });
    }
